...
async def next_page(bot, query):
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://t.me/animecreationau')
        ] 
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("Ne enne patikkan nokkuno.Ne kollalo 😒", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
            return await query.answer('Enjanathe file kaanan ella ketto.')
        files = files_[0]
        title = files.file_name
        size = get_size(files.file_size)
        f_caption = files.caption
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🔔 Updates 🤖', url='https://t.me/auoffc')
        ], [
            InlineKeyboardButton('🙆🏻 Help 🦾', callback_data='help'),
            InlineKeyboardButton('♥️ About ♥️', callback_data='about')
        ],[
            InlineKeyboardButton('Yt channel', url='https://youtube.com/@Auoffcial')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('🔔 Updates 🤖', url='https://t.me/aoffc'),
        ...
        try:
            btn = [[
                ...
                InlineKeyboardButton(text=f"🐞 REPORT BUG 🐞", url=f"https://t.me/+3zF9tC_HoQEyZGVl")
            ],[
                InlineKeyboardButton(text=f"⚡️ Yt channel 🦋", url=f"https://youtube.com/@Auoffcial")
        ...
        try:
            btn = [[
                ...
                InlineKeyboardButton(text=f"⚡️ Yt channel 🦋", url=f"https://youtube.com/@Auoofcial")
            ]]
            btn_lzdv = [
                [
                InlineKeyboardButton(text=f"🗑 Delete Log ❌", callback_data = "close_data")
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ to ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://t.me/animecreationau'),
        ] 
    )
    if offset != "":
...
async def advantage_spell_chok(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("I couldn't find any movie in that name. E nameil oru cinemayumkitilla.Correct spelling anno enni check chaithu nokku.")
        await asyncio.sleep(10)
        await k.delete()
        return
